...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-xl border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        default: "border-slate-800 bg-vault-card text-slate-100",
        destructive:
          "destructive group border-red-900/50 bg-red-900/20 text-red-100",
        success: "border-green-900/50 bg-green-900/20 text-green-100",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-md border border-slate-700 bg-transparent px-3 text-sm font-medium ring-offset-vault-navy transition-colors hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-vault-blue focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 group-[.destructive]:border-red-800 group-[.destructive]:hover:border-red-700 group-[.destructive]:hover:bg-red-800 group-[.destructive]:hover:text-red-100 group-[.destructive]:focus:ring-red-600",
      className
    )}
    {...props}
  />
